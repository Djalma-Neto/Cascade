...
for x in range(len(os.listdir("positivas"))):
    try:
        ...
        comando = "opencv_createsamples -img ./positivas/{0} -bg bg.txt -info {1} -pngoutput info -maxxangle 0.5 -maxyangle 0.5 -maxzangle 0.5 -num {2}".format(img,infoDir,num)
        os.system(comando)
    except Exception as e:
        print(str(e))
...
if(len(os.listdir("info")) > 1):
    for x in range(len(os.listdir("info"))):
        try:
            ...
            infoDir = "info/{0}/info.lst".format(x)
            comando = "opencv_createsamples -info {0} -num {1} -w 20 -h 20 -vec vecs/positives{2}.vec".format(infoDir,num,x)
            os.system(comando)
        except Exception as e:
            print(str(e))
    ...
    comando = "opencv_createsamples -info info/0/info.lst -num {0} -w 20 -h 20 -vec positivas.vec".format(num)
    os.system(comando)